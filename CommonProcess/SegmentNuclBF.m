...
GrowBoundingBox = 80; Var.SegmentationParameters(ParaNum).BoundingBox; %30;
...
if NumNucl > 0
    for N = 1:NumNucl
        ...
        % assignin('base',['CutNorm', num2str(N)], CutNorm)
        
        %Use active contour to find Cell shape in BF image
        SegOK = 0;
        NuclGrow = 2;
        ContourIter = 100;
        %Run loop until sggmentation is OK
       while SegOK == 0 && NuclGrow < 6 %&& ContourIter >10
           %Dilate Nucleus (Dilation increases at each iteration
            SE = strel('disk',NuclGrow);
            CutNuclDil = imdilate(CutNucl,SE);
            CutSeg = double(activecontour(CutNorm, CutNuclDil, ContourIter));
            % assignin('base',['CutSeg', num2str(N)], CutSeg)
            %Get  properties
            PropsSeg = regionprops(CutSeg, 'ConvexArea', 'Area', 'MajorAxisLength','MinorAxisLength', 'Eccentricity','Solidity');
            NuclDilArea = length(find(CutNucl>0));
            %Test if size of object is too large
           if PropsSeg(1).MajorAxisLength > MaxDiam
               fmmThresh = 0.1;
               % use binary segmentation to find a smaller contour
                while PropsSeg(1).MajorAxisLength > MaxDiam && fmmThresh > 0.001
                    CutSeg = double(imsegfmm(CutNorm, im2bw(CutNuclDil), fmmThresh));
                     PropsSeg = regionprops(CutSeg, 'ConvexArea', 'Area', 'MajorAxisLength','MinorAxisLength', 'Eccentricity','Solidity');
                     fmmThresh = fmmThresh/2;
              ...
              %Test if found object is 3 times larger than Nucl If not try
              %again
           elseif PropsSeg(1).ConvexArea < 3*NuclDilArea
                NuclGrow = NuclGrow+1;
            else
                SegOK = 1;
            end
            ...
            if Debug
                figure(3000+N); title(num2str(NuclGrow))
                % CutSegFilter = double(CutSegFilter);
                % CutSegFilter(CutSeg)= 3;
                subplot(2,2,1); imagesc(CutNucl), title(['CutNucl ', num2str(N),'Dil ', num2str(NuclGrow), 'Iter ', num2str(ContourIter), ' ',num2str(NuclDilArea)  ])
                
                subplot(2,2,2); imagesc(CutZOff), title(['CutZOff Solidity: ',num2str(PropsSeg(1).Solidity) ,' Eccen ', num2str(PropsSeg(1).Eccentricity)])
                subplot(2,2,3); imagesc(CutFocus), title(['CutFocus Axese: ',num2str(PropsSeg(1).MajorAxisLength) ,' - ', num2str(PropsSeg(1).MinorAxisLength)])
                subplot(2,2,4); imagesc(CutSeg), title(['CutSeg Area ',num2str(PropsSeg(1).Area) ,' CA', num2str(PropsSeg(1).ConvexArea)])
                pause(1)
            end
            
        end
        
        % PROBLEM IF NO AREA FOUND FOR NOW SET object TO NUCL AREA
            if max(CutSeg(:)) ==0
                CutSeg = CutNuclDil;
        ...
        CutSegFocus = double(activecontour(CutFocusNorm, CutConvDil, 100));
        ...
        if Debug
                figure(4000+N);
                subplot(2,2,1); imagesc(CutConvDil+ 2*CutConvDilHi), title(['CutConvDil'])
                
                subplot(2,2,2); imagesc(CutSegFocus), title('CutSegFocus')
